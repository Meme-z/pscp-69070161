...
```
Input: word
Output: reversed version of that word (Ex. cat -> tac)
Constraints:
    the output reversed word must be in lowercase
```
...
Step 3: Print the reversed word (all letters in lowercase)
...
```
I use .lower() for making all letters in word lowercase
I use [::-1] to reverse the word
```
...
```
This testcase for checking if it print correct word
...
```
This Testcase for checking if my code print word in lowercase
